...
export function database(){const db=(env as unknown as {DB:D1Database}).DB;if(!db)throw new Error("数据库暂不可用，请稍后重试。");return db}
export function bucket(){const b=(env as unknown as {BUCKET:R2Bucket}).BUCKET;if(!b)throw new Error("图片存储暂不可用，请稍后重试。");return b}
...
export async function isOwner(){const user=await getChatGPTUser();return !!user&&(user.userId===OWNER_ID||user.email?.toLowerCase()==="wjy2378408967@gmail.com")}
